...
from settings import IMG_HEIGHT, IMG_WEDTH
...
class Table:
    ...
    @staticmethod
    def save(name, sorted_images: list) -> None:
        """Создает таблицу с полученного списка."""
    ...
    def make_sorting(self, path: pathlib.Path) -> list:
        ...
        for elem in images:
            ...
            predictions = self.ml.model.predict(img_array, verbose=0)
